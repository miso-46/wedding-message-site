...
結婚式のゲスト一人ひとりに、個別のメッセージを届けるデジタルメッセージサイトです。
...
本番運用する場合は、`public/data.json` にゲスト名とメッセージを登録し、Vercel などにデプロイしてQRコードを配布します。スプレッドシートで氏名・フリガナ・メッセージをまとめておき、ChatGPT などに「JSON形式にして」と頼むと楽です。
